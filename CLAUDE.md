...
**Current state:** Milestone 1 complete (Stages 1-4), Milestone 2 in progress. Implemented:
`tracing.py`, `settings.py`, `hello.py`, `contracts.py`, Stages 1-7 under `ingest/` (Stages 6 and
7 share `s06_s07_index.py`), `stores/embedding_cache.py`, `stores/parent_store.py`, `ingest/run.py`,
`flp ingest [--only <stage>] [--force]`, and `models.py`. Stage 7 talks to Qdrant through
`qdrant-client` directly: collection `flp_chunks_{index_version}`, named vectors `dense` and
`sparse_bm25`, point id = UUID5 of `chunk_id`, payload from Stage 5. It never touches the alias;
Stage 8 (#14) verifies, switches `flp_chunks_current`, and prunes old collections. Parent store
and `manifest.json` live in `data/index/{index_version}/`. Next: Stage 8 (#14).
