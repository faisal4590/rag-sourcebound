...
**Current state:** Milestone 2 complete. Ingestion Stages 1-8 under `ingest/`, `make ingest`
ends with `VERIFY OK: 10/10 smoke queries hit`, alias `flp_chunks_current` points at the verified
collection. Query side so far: Stage 12 dense retrieval in `graph/nodes/retrieve.py`
(`retrieve_dense`, `RetrievalFilters`, `display_texts`) and `flp ask "<question>" [--chapter N]
[--code/--no-code] [--top-k K]`, which prints chunk id, chapter, printed pages, cosine score, and
the first 200 characters of display text read from the parent store. No answer generation yet.
Next: Milestone 3, LangGraph wiring (#16) and the FastAPI app (#17).
