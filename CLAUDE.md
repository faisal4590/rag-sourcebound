...
`tracing.py`, `settings.py`, `hello.py`, `contracts.py`, Stages 1-4 under `ingest/`, `ingest/run.py`,
`flp ingest [--only parse|structure|clean|chunk] [--force]`, and `models.py` (provider layer:
`BgeM3Embedder`, `OpenAIEmbedder`, `FastembedSparse`, `BgeReranker`, `CohereReranker`,
`LangChainChat`, `Secrets`). Shipped PDF: 1,529 blocks, 279 chunks, 180 parents. Local models
`BAAI/bge-m3`, `BAAI/bge-reranker-v2-m3`, and `Qdrant/bm25` live in the Hugging Face and fastembed
caches; tests that need them skip when absent. Next: Stage 5 enrichment (#11).
