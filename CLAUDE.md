...
`tracing.py`, `settings.py`, `hello.py`, `contracts.py`, Stages 1-5 under `ingest/`, `ingest/run.py`,
`flp ingest [--only parse|structure|clean|chunk|enrich] [--force]`, and `models.py` (provider
layer). Shipped PDF: 1,529 blocks, 279 chunks, 180 parents, 498 distinct `php_symbols`. Stage 5
decides `index_version` (`v{n}-bgem3-{chunk_config_hash[:8]}`, `n` from `data/index/`) and Stage 7
reads it from the payload. Local models live in the Hugging Face and fastembed caches (see
below). Next: Stage 6 embedding (#12).
