...
`Chapter`, `ParsedPage`, `Rect`), Stage 1 `ingest/s01_parse.py` (words, glyph repair, shaded
rectangles, chapter table), Stage 2 `ingest/s02_structure.py` (typed blocks), Stage 3
`ingest/s03_clean.py` (artifact rules with counts), `ingest/run.py`, and
`flp ingest [--only parse|structure|clean] [--force]`. Shipped PDF: 313 pages, 45,456 words,
1,529 blocks (33 chapter titles, 127 headings, 36 callouts, 400 code, 933 paragraphs). Stage 3
fires no rule on this edition; the text is already NFC and header-free. Next: Stage 4 (#9).
