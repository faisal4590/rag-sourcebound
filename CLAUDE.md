...
**Current state:** Milestones 0-2 ingestion complete: Stages 1-8 under `ingest/`, `ingest/run.py`,
`flp ingest [--only <stage>] [--force]` (exit 1 when verification fails), `models.py`,
`stores/embedding_cache.py`, `stores/parent_store.py`. `make ingest` ends with `VERIFY OK: 10/10
smoke queries hit` and the alias `flp_chunks_current` points at the verified collection; the
previous collection stays for one rollback and older ones are pruned. A second `flp ingest`
reports `already indexed`. Verification flags one thin chapter (First-class callables, 1 chunk
over 4 pages); flags never fail a run. Next: Stage 12 dense retrieval and `flp ask` (#15).
