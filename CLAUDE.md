...
`tracing.py`, `settings.py`, `hello.py`, `contracts.py`, Stages 1-6 under `ingest/` (Stage 6 is
the embedding half of `s06_s07_index.py`), `stores/embedding_cache.py`, `ingest/run.py`,
`flp ingest [--only parse|structure|clean|chunk|enrich|embed] [--force]`, and `models.py`.
Shipped PDF: 1,529 blocks, 279 chunks, 180 parents, 279 dense (1024) plus sparse vectors in
`data/vectors/`, dense vectors cached in `data/cache/embeddings.sqlite`. Embedding the book with
bge-m3 on this CPU takes about 4 minutes; a rerun with unchanged text costs zero model calls.
Next: Stage 7 indexing into Qdrant (#13).
