...
Stage 12 dense retrieval in `graph/nodes/retrieve.py`, and `flp ask` (retrieval only). Default
...
(#22), respond minimal (#23). `trace.instrument_langchain` (default false) adds OpenInference
LangChain spans to the same provider. Next: the FastAPI app (#17).
