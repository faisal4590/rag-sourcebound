...
**Current state:** Milestone 1 complete (Stages 1-4). Implemented: `tracing.py`, `settings.py`,
`hello.py`, `contracts.py`, Stage 1 `s01_parse.py` (words, glyph repair, shaded rectangles,
chapter table), Stage 2 `s02_structure.py` (typed blocks), Stage 3 `s03_clean.py` (artifact
rules), Stage 4 `s04_chunk.py` (chunks and parents), `ingest/run.py`, and
`flp ingest [--only parse|structure|clean|chunk] [--force]`. Shipped PDF: 1,529 blocks, about 280
chunks (max 742 tokens, none over 900), about 180 parents. Chunking decisions that bend the spec:
a section under 120 tokens is one chunk (46 of 160 sections), a callout is its own chunk, and
`class CustomerDTO` appears in 13 code blocks so the grep test pins the page 66 chunk. Next:
Milestone 2, `models.py` (#10) and Stage 5 (#11).
